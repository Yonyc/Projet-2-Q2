...
def trouver_race(db: sqlite3.Connection, animal_id: int):
    ...
    if not len(race_mere):
        race_mere: List[Tuple[int, float]] = trouver_race(db, mere)

    if not len(race_pere):
        race_pere: List[Tuple[int, float]]= trouver_race(db, pere)
